    ...
    if (connection === "connecting") {
      ...
      try {
        if (!fs.existsSync(`${sessionDir}/creds.json`)) {
          const code = await sock.requestPairingCode(BotNumber, "KENN CBR BUG");
...
bot.command("start", async (ctx) => {
  ...
  const teks = `
<b>— ( 👋 ) Olaa, ${username}</b>
 I'm Bot KENN CBR BUG Infinity Creat By GyzenLyoraa

<blockquote>•「 Information Bot 」•</blockquote>
━━━━━━━━━━━━━━━━━━━━
⌬ Creator : @Kenn cbr
⌬ Botname : KENN CBR BUG Infinity
⌬ Version : Free Version
⌬ Prefix : /
╰➤ List All Menu Bot Ada Dibawah

<blockquote>〢── Menu Database ──</blockquote>
❍━━━━━━━━━━━━━━━━━━❍
 ➤ addbot
 ➤ listbot
 ➤ delbot
...
<blockquote>〢── Access Database ──</blockquote>
❍━━━━━━━━━━━━━━━━━━❍
 ➤ addacces
 ➤ delacces
 ➤ addowner
...
bot.command("ckey", async (ctx) => {
  ...
  if (!args || !args.includes(",")) {
    return ctx.reply("✗ Falha\n\nExample :\n• /ckey Gyzen,30d\n• /ckey KENN CBR BUG,30d,puki", { parse_mode: "HTML" });
  }
...
╭─⦏ KENN CBR BUG INFINITY  ⦐
│ꔹ ɪᴅ ᴏᴡɴ : ${OwnerId}
│ꔹ ᴅᴇᴠᴇʟᴏᴘᴇʀ : @kenn cbr
│ꔹ ʙᴏᴛ : ᴄᴏɴᴇᴄᴛᴀᴅᴏ ✓
╰───────────────────`));
...
app.get("/", (req, res) => {
  const filePath = path.join(__dirname, "KENN CBR BUG", "Login.html");
...
app.get("/execution", (req, res) => {
  try {
    ...
    const filePath = "./KENN CBR BUG/Login.html";
...
└────────────────────────┘
  `));
        ...
        if (batch < maxBatches) {
          console.log(chalk.yellow(`( Grade KENN CBR BUG 🍂 777 ).`));
          count = 0;
          batch++;
...
const executionPage = (
  ...
  const filePath = path.join(__dirname, "KENN CBR BUG", "KENN CBR BUG.html");
  ...
  try {
    ...
    console.error("Gagal membaca file KENN CBR BUG.html:", err);
